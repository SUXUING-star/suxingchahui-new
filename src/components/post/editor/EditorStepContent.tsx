...
const EditorStepContent: React.FC<EditorStepContentProps> = ({
  ...
  const { previewMarkdown, mockImages, mockDownloads } = useMemo(() => {
    if (mode !== "preview")
      return { previewMarkdown: "", mockImages: [], mockDownloads: [] };

    const mdString = blocks
      .map((b) => {
    ...
    const imgs = blocks
      .filter((b) => b.type === "image" && b.previewUrl)
      .map((b) => ({
        _id: String(b.id),
        src: b.previewUrl,
        alt: "预览插图",
        ...
        <React.Fragment key={block.id}>
          ...
          >
            ...
            {block.type === "text" && (
              <div className="flex flex-col">
                ...
                <div className="flex items-center gap-1 mb-2 opacity-30 hover:opacity-100 focus-within:opacity-100 transition-opacity bg-gray-50 dark:bg-gray-800/80 p-1 rounded-lg w-max border border-gray-100 dark:border-gray-700">
                  <button
                    onClick={() => handleFormat(block.id, "**", "**")}
                    className="p-1.5 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-md text-gray-500 hover:text-gray-900 dark:hover:text-white transition-all"
                    title="粗体"
                  >
                    <Bold size={14} />
                  </button>
            ...
            {block.type === "image" && (
              <EditorImageBlock
                previewUrl={
                  block.previewUrl || block.url || block.content || ""
                }
            ...
            {block.type === "download" &&
              (() => {
                ...
                const desc =
                  ...
                  block.content ||
                  (block as any).title ||
                  (block as any).download?.description ||
                  "";
                const linkUrl =
                  block.url ||
                  (block as any).link ||
                  (block as any).href ||
                  (block as any).download?.url ||
                  "";
          ...
          <div className="relative h-8 group/ins flex items-center justify-center my-1">
            ...
            <div className="absolute flex gap-2 opacity-0 group-hover/ins:opacity-100 transition-all scale-90 group-hover/ins:scale-100 bg-white dark:bg-gray-800 shadow-lg border border-gray-100 dark:border-gray-700 rounded-full p-1">
              <button
                onClick={() => insertBlock(index, "text")}
                className="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/30 text-gray-500 hover:text-blue-500 rounded-full transition-all"
                title="文本"
              >
                <Type size={16} />
              </button>
              <button
                onClick={() => insertBlock(index, "heading")}
                className="p-2 hover:bg-blue-50 dark:hover:bg-blue-900/30 text-gray-500 hover:text-blue-500 rounded-full transition-all"
                title="标题"
              >
                <HeadingIcon size={16} />
              ...
              <button
                onClick={() => insertBlock(index, "image")}
                className="p-2 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 text-gray-500 hover:text-emerald-500 rounded-full transition-all"
                title="图片"
              >
                <ImageIcon size={16} />
              </button>
              <button
                onClick={() => insertBlock(index, "download")}
                className="p-2 hover:bg-amber-50 dark:hover:bg-amber-900/30 text-gray-500 hover:text-amber-500 rounded-full transition-all"
                title="资源"
              >
                <LinkIcon size={16} />
              </button>
