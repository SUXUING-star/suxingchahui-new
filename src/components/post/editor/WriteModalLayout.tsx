...
const WriteModalLayout: React.FC<WriteModalLayoutProps> = ({
  ...
  // 进场动画
  useEffect(() => {
    // 2. 增加 TypeScript 非空保护，更改为 v4 参数结构
    if (modalRef.current) {
  ...
  // 退出动画并关闭
  const handleClose = () => {
    // 3. 增加非空保护，complete 重命名为 onComplete，并应用 v4 的 ease
    if (modalRef.current) {
      animate(modalRef.current, {
        opacity: [1, 0],
        scale: [1, 0.95],
        ...
        {isLoadingData ? (
          ...
          <>
            ...
            <div className="px-8 py-4 border-b border-gray-100 dark:border-white/10 flex justify-between items-center bg-white/50 dark:bg-black/20 flex-shrink-0">
              <div className="flex items-center space-x-4">
                {steps.map((s) => (
                  ...
                  >
                    <div
                      className={`w-6 h-6 rounded-full flex items-center justify-center font-bold text-xs shadow-sm transition-all ${
                        s.isDone
                    ...
                    <span
                      className={`font-bold text-xs ${step === s.id ? "text-blue-600 dark:text-blue-400" : "text-gray-400"}`}
                    >
                      {s.title}
                    </span>
                  </button>
              ...
              <div className="flex items-center space-x-3">
                {isAdmin && (
                  ...
                  >
                    <Bookmark
                      size={14}
                      className={`mr-2 transition-all ${isTopped ? "fill-red-600 scale-110" : ""}`}
                    />
                    {isTopped ? "已置顶" : "设为置顶"}
                  </button>
                )}
                ...
                <button
                  ...
                  className="flex items-center px-4 py-2 rounded-xl bg-white dark:bg-gray-800 font-bold text-xs hover:bg-gray-50 dark:hover:bg-gray-700 transition-all border border-gray-200 dark:border-gray-700 shadow-sm"
                >
                  {mode === "edit" ? (
                    <>
                      <Eye size={14} className="mr-2 text-blue-500" /> 预览
                    </>
                  ) : (
                    <>
                      <Edit3 size={14} className="mr-2 text-green-500" /> 编辑
                    </>
                  )}
                </button>
                <button
            ...
            <div className="px-8 py-6 bg-white/80 dark:bg-black/40 border-t border-gray-100 dark:border-white/5 flex items-center justify-between flex-shrink-0 backdrop-blur-md">
              ...
              >
                <ChevronUp size={20} /> <span>上一步</span>
              </button>
              ...
              {step < 3 ? (
                ...
                >
                  <span>下一步</span> <ChevronDown size={20} />
                </button>
              ) : (
                <button
                  onClick={handleFinalPublish}
                  disabled={isSubmitting}
                  className="px-8 py-3 bg-blue-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-blue-500/30 hover:scale-105 active:scale-95 transition-all flex items-center disabled:opacity-50 disabled:hover:scale-100"
                >
                  {isSubmitting ? (
