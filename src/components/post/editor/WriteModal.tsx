...
// 引入 PostRequest 接口以及配套的工厂函数 createPostRequest
...
const WriteModal: React.FC<WriteModalProps> = ({
  ...
  const [blocks, setBlocks] = useState<Block[]>([
    { id: Date.now(), type: "text", content: "", invalid: false },
  ]);
  ...
  const isContentDone = blocks.some(
    (b) =>
  ...
  const loadExistingPost = useCallback(async () => {
    ...
    try {
      ...
      setTags(post.tags);
      setTopped(post.topped);
      setCover({ file: null, preview: post.coverImage?.src || "" });

      const parsedBlocks = parseMDToBlocks(
        post.content,
        post.contentImages,
        post.downloads,
      );
      const sanitizedBlocks: Block[] = parsedBlocks.map((b) => ({
        ...
        resourceId: b.resourceId || (b as any)._id,
      }));
  ...
  const handleImageSelect = async (
    ...
    try {
      const { file: convertedFile, previewUrl: finalPreview } =
  ...
  const handleFinalPublish = async () => {
    if (!isMetaDone) {
      setStep(1);
      return showNotification("核心配置未完成", "error");
    }
    if (!isCoverDone) {
      setStep(2);
      return showNotification("请设置封面", "error");
    }
    if (!isContentDone) {
      setStep(3);
      return showNotification("文章内容不能为空", "error");
    }

    const brokenLinks = blocks.filter(
      (b) => b.type === "download" && (!b.url || b.url.trim() === ""),
    );
    if (brokenLinks.length > 0) {
      setBlocks((prev) =>
        prev.map((b) =>
          b.type === "download" && !b.url ? { ...b, invalid: true } : b,
        ),
      );
      setStep(3);
      return showNotification(
        "存在未填写的下载链接，请检查或删除该块后再提交",
        "error",
      );
    }
    ...
    try {
      let coverData = cover.preview
        ? { src: cover.preview, alt: title }
        : undefined;
      if (cover.file) {
      ...
      const mdContent = finalBlocks
        .map((b) => {
          if (b.type === "heading") return `### ${b.content}`;
          if (b.type === "image" && b.resourceId)
            return `[image:${b.resourceId}]`;
          if (b.type === "download" && b.resourceId)
            return `[download:${b.resourceId}]`;
          return b.content;
        })
        .join("\n\n");

      // 替代 new PostRequest() 写法，调用工厂函数
      const requestModel = createPostRequest({
        title,
        category,
        tags,
