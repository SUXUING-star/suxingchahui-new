...
const EditorImageBlock: React.FC<EditorImageBlockProps> = ({
  previewUrl,
  onSelect,
  onRemove,
}) => {
  ...
  return (
    <div className="w-full group/img">
      {previewUrl ? (
        ...
        >
          <div
            className={`w-20 h-20 bg-white dark:bg-gray-800 rounded-3xl shadow-2xl flex items-center justify-center mb-6 transition-all duration-500 ${isDragging ? "scale-125 rotate-12 bg-blue-500 text-white" : "group-hover/img:scale-110"}`}
          >
            {isDragging ? (
          ...
          <div className="text-center space-y-1 pointer-events-none">
            <p
              className={`text-lg font-black tracking-tighter uppercase transition-colors ${isDragging ? "text-blue-600" : "text-gray-900 dark:text-white"}`}
            >
