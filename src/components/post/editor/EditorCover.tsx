      ...
      >
        {preview ? (
          <>
            ...
            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-all flex items-center justify-center backdrop-blur-sm">
              <button
                ...
                className="p-4 bg-red-600 text-white rounded-2xl hover:scale-110 transition-all shadow-xl font-black text-xs uppercase"
              >
                更换图片
              </button>
            </div>
          ...
          <div className="flex flex-col items-center justify-center h-full pointer-events-none">
            <div
              className={`p-6 bg-white dark:bg-gray-800 rounded-3xl shadow-2xl mb-4 transition-all duration-500 ${isDragging ? "scale-125 rotate-6 bg-blue-600 text-white" : "group-hover:scale-110"}`}
            >
              {isDragging ? (
            ...
            <span
              className={`text-xs font-black uppercase tracking-widest transition-colors ${isDragging ? "text-blue-600" : "text-gray-500"}`}
            >
