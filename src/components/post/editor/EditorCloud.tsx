...
const EditorCloud: React.FC<EditorCloudProps> = ({
  id,
  title,
  isDone,
  isActive,
  ...
  return (
    <div className="w-full h-full flex flex-col animate-in fade-in zoom-in-95 duration-500">
      <div className="flex items-center space-x-5 mb-10">
        <div
          className={`w-14 h-14 rounded-3xl flex items-center justify-center font-black text-2xl shadow-2xl transition-all ${isDone ? "bg-green-500 text-white scale-110" : "bg-blue-600 text-white"}`}
        >
          {isDone ? <Check size={28} strokeWidth={4} /> : id}
        </div>
        <div>
          <h3 className="text-sm font-black text-gray-400 uppercase tracking-[0.5em]">
            {title}
          </h3>
          {!isDone && (
