...
interface ReplyItemProps {
  ...
  slug: string;
  onRefresh: () => void;
  onOpenDeleteModal: (comment: CommentResponse) => void;
  depth?: number;
}
...
const ReplyItem: React.FC<ReplyItemProps> = ({
  ...
  const canDelete =
    isAuthenticated &&
    user &&
    (user.isAdmin || user.nickname === comment.user.nickname);

  return (
    <div
      className={`mt-6 ${depth > 0 ? "ml-4 sm:ml-12 border-l-2 border-gray-100 dark:border-white/5 pl-4 sm:pl-8" : ""}`}
    >
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center space-x-5 mt-2 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              onClick={() =>
                isAuthenticated ? setIsReplying(!isReplying) : openAuthModal()
              }
            ...
            <div className="mt-4 flex flex-col space-y-2 animate-in slide-in-from-top-2">
              ...
              <div className="flex justify-end space-x-2">
                <button
                  ...
                  className="px-4 py-1.5 text-[10px] font-black text-gray-400"
                >
                  取消
                </button>
                <button
