...
const getTotalCount = (list) => {
  let count = 0;
  const walk = (items) => {
    items.forEach(i => { count++; if(i.replies) walk(i.replies); });
  };
  walk(list || []);
  return count;
};

const CommentSection = ({ comments, slug, onRefresh }) => {
  ...
  const [deleteConfig, setDeleteConfig] = useState({ isOpen: false, comment: null });
  ...
  return (
    <section className="w-full">
      ...
      <div className="mb-16 relative">
        <textarea 
          rows={4} value={mainContent} onChange={e => setMainContent(e.target.value)}
      ...
      <div className="space-y-8">
        {comments.length > 0 ? (
          comments.map(c => (
            <CommentItem 
              key={c._id} comment={c} slug={slug} onRefresh={onRefresh} 
              onOpenDeleteModal={(comment) => setDeleteConfig({ isOpen: true, comment })} 
            />
          ))
        ) : (
