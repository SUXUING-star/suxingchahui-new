...
const BookDetailModal: React.FC<Props> = ({ book, onClose, onRefresh }) => {
  ...
  const handleUpdate = async () => {
    setLoading(true);
    try {
      await updateBook(book.id, formData, token);
      setIsEditing(false);
  ...
  const updateStory = (idx: number, val: string) => {
    const s = [...formData.stories];
    s[idx] = val;
    setFormData({ ...formData, stories: s });
  };
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
      ...
      <div
        ref={modalRef}
        className="relative w-full max-w-3xl bg-white dark:bg-[#0a0a0a] rounded-[40px] shadow-2xl border border-white/10 flex flex-col max-h-[90vh] overflow-hidden"
      >
        {/* 固定头部岛屿 */}
        <header className="p-6 md:p-10 pb-4 flex justify-between items-start shrink-0">
          <div className="flex-1 space-y-4">
            <div className="flex items-center gap-3">
              ...
              {isEditing ? (
                ...
                <span
                  className={`flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-tighter border ${
            ...
            {isEditing ? (
              ...
              <h2 className="text-2xl md:text-4xl font-black italic text-blue-600 tracking-tight">
                {formData.title}
              </h2>
            )}
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-3 text-[10px] font-black text-gray-500 uppercase tracking-widest">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {isEditing ? (
                  ...
                  BOOK_TYPES.find((t) => t.id === formData.bookType)?.label ||
                  "未知体裁"
                )}
              </span>
              ...
              <span className="flex items-center gap-1.5">
                ...
                {isEditing ? (
                  <input
                    className="bg-transparent border-b border-gray-600 w-16 outline-none"
                    value={formData.year}
                    onChange={(e) =>
              ...
              <span className="flex items-center gap-1.5">
                <Clock size={14} />{" "}
                {new Date(book.createdAt).toLocaleDateString()} 记录
              </span>
            </div>
          </div>
          ...
          <div className="flex gap-2 ml-4">
            <button
              ...
              className={`p-3 rounded-xl transition-all ${isEditing ? "bg-orange-500 text-white shadow-lg" : "bg-gray-100 dark:bg-white/5 hover:scale-110"}`}
            >
              {isEditing ? <RotateCcw size={20} /> : <Edit3 size={20} />}
            </button>
            <button
              onClick={handleClose}
              className="p-3 hover:bg-red-500 hover:text-white dark:bg-white/5 rounded-xl transition-all"
            >
              <X size={20} />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 md:p-10 pt-2 space-y-10 custom-scrollbar">
            ...
            <section className="bg-blue-600/5 rounded-[32px] p-6 border border-blue-500/10">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5">
                {formData.stories.map((story, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-2 bg-white dark:bg-black/20 p-2 rounded-xl border border-white/10 shadow-sm"
                  >
                    {isEditing ? (
                      <>
                        <input
                        ...
                        <button
                          onClick={() =>
                            setFormData((p) => ({
                              ...p,
                              stories: p.stories.filter((_, i) => i !== idx),
                            }))
                          }
                      ...
                      <span className="text-[11px] font-bold text-gray-500 truncate px-2">
                        {story}
                      </span>
                    )}
                  </div>
                ...
                {isEditing && (
                  <button
                    ...
                    className="flex items-center justify-center gap-2 border-2 border-dashed border-blue-500/20 rounded-xl py-2 text-blue-500 hover:bg-blue-500/5 transition-all text-[10px] font-black"
                  >
                    <Plus size={14} /> 新增篇目
                  </button>
                )}
          ...
          <section className="space-y-4">
            <h4 className="flex items-center gap-2 text-[10px] font-black text-blue-500 uppercase tracking-widest">
              <BookOpen size={14} /> 阅读随笔
            </h4>
            {isEditing ? (
              <textarea
                className="w-full text-sm font-bold text-gray-700 dark:text-gray-200 bg-gray-50 dark:bg-white/5 p-6 rounded-3xl border-none outline-none focus:ring-2 ring-blue-500/30 min-h-[120px]"
          ...
          <section className="space-y-4">
            <h4 className="flex items-center gap-2 text-[10px] font-black text-emerald-500 uppercase tracking-widest">
              <MessageSquare size={14} /> 详细记录
            </h4>
            {isEditing ? (
              <textarea
                className="w-full text-sm leading-relaxed text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-white/5 p-6 rounded-3xl min-h-[250px] outline-none focus:ring-2 ring-emerald-500/30"
                value={formData.longReview}
                onChange={(e) =>
                  setFormData({ ...formData, longReview: e.target.value })
                }
                placeholder="录入深度的分析内容..."
              />
            ) : (
