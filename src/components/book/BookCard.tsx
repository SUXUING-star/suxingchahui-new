...
const BookCard: React.FC<BookCardProps> = ({
  ...
  return (
    <div
      onClick={() => (isSelectMode ? onToggleSelect(book.id) : onClick(book))}
    ...
    >
      ...
      <div
        className={`absolute -top-1.5 -right-1 z-10 px-2 py-0.5 rounded-sm shadow-md transform rotate-6 border border-white/20 flex items-center gap-1 ${
          isRead ? "bg-emerald-500 text-white" : "bg-orange-500 text-white"
        }`}
      >
      ...
      {/* 2. 书脊渐变条：模拟圆润书脊的明暗效果 */}
      <div
        className={`absolute top-0 left-0 bottom-0 w-[4px] md:w-[6px] rounded-l-sm ${
          isRead
      ...
      {isSelectMode && (
        <div className="absolute top-3 right-4 z-20">
          <div
            className={`w-4 h-4 md:w-6 md:h-6 rounded-md border-2 flex items-center justify-center transition-all ${
              isSelected
      ...
      {/* 标题 */}
      <h3
        className={`${getTitleFontSize(book.title)} font-serif font-black tracking-tight line-clamp-1 uppercase mb-1 md:mb-2 group-hover:text-blue-600 transition-colors pr-8`}
      >
        {book.title}
      </h3>
      ...
      <div className="flex flex-col gap-0.5 md:gap-1.5 mb-2 md:mb-4">
        <div className="flex items-center text-[7px] md:text-[9px] font-black text-gray-400 uppercase tracking-tighter truncate">
          <User
            size={8}
            className="mr-1 text-blue-500 shrink-0 md:w-3 md:h-3"
          />
        ...
        <div className="flex items-center text-[7px] md:text-[9px] font-black text-gray-400 uppercase tracking-tighter truncate">
          <Globe
            size={8}
            className="mr-1 text-emerald-500 shrink-0 md:w-3 md:h-3"
          />
      ...
      {book.shortReview && (
        <p
          className="text-[10px] md:text-[11px] leading-relaxed text-gray-500 dark:text-gray-400
      ...
      {/* 底部栏 */}
      <div
        className={`pt-1.5 md:pt-3 border-t flex items-center justify-between ${isRead ? "border-emerald-100 dark:border-emerald-900/20" : "border-orange-100 dark:border-orange-900/20"}`}
      >
        <span
          className={`flex items-center gap-1 text-[7px] md:text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md border ${
            isRead
