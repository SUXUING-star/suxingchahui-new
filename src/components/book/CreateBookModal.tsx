...
import {
  BOOK_COUNTRIES, BOOK_TYPES, BOOK_TYPE_MAP,
  createBook, createBooksBatch, INITIAL_BOOK_FORM,
} from '@/utils/bookApi';
...
const CreateBookModal: React.FC<CreateBookModalProps> = ({ onClose, onSuccess }) => {
  ...
  useEffect(() => {
    ...
    anime({ targets: modalRef.current, scale: [0.9, 1], translateY: [40, 0], opacity: [0, 1], duration: 800, easing: 'easeOutElastic(1, .8)' });
  }, []);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      <div ref={backdropRef} className="absolute inset-0 bg-black/70 backdrop-blur-md" onClick={handleClose} />

      <div ref={modalRef} className="relative w-full max-w-2xl bg-white dark:bg-[#121212] rounded-[32px] shadow-2xl border border-white/10 flex flex-col max-h-[90vh]">
        <header className="p-6 pb-2 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-3">
            <div className="w-1.5 h-6 bg-blue-600 rounded-full"></div>
            <h2 className="text-xl font-black tracking-tight">{isBatchMode ? '批量导入' : '记录新书'}</h2>
          </div>
          <div className="flex gap-2">
            ...
            <button onClick={handleClose} className="p-2 text-gray-400 hover:bg-white/5 rounded-full"><X size={20} /></button>
          </div>
        </header>
          ...
          {isBatchMode ? (
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                />
                <button type="button" onClick={handleParse} className="w-full py-3.5 bg-gray-900 dark:bg-white text-white dark:text-gray-900 rounded-xl font-black text-xs shadow-lg active:scale-95 transition-all">识别并预览记录</button>
              </div>

              {batchPreview.length > 0 && (
                <div className="space-y-4 pb-4">
                  <label className="text-[10px] font-black uppercase text-blue-600 px-1">识别预览 ({batchPreview.length} 条)</label>
                  <div className="space-y-3">
                    {batchPreview.map((item, idx) => (
                      <div key={idx} className="bg-gray-50 dark:bg-white/3 p-5 rounded-[24px] border border-gray-100 dark:border-white/5 space-y-4 relative">
                        <button type="button" onClick={() => setBatchPreview(p => p.filter((_, i) => i !== idx))} className="absolute top-4 right-4 text-gray-300 hover:text-red-500"><Trash2 size={16} /></button>
                        ...
                        <div className="grid grid-cols-3 gap-4">
                          ...
                          <select className="bg-transparent border-b border-gray-200 dark:border-white/10 text-[10px] font-bold outline-none cursor-pointer" value={item.bookType} onChange={e => handleBatchUpdate(idx, 'bookType', e.target.value)}>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-gray-500 flex items-center gap-2 px-1"><Type size={12} className="text-blue-600" /> 书籍名称</label>
                  <input required className="w-full bg-gray-100 dark:bg-white/5 border-none rounded-xl px-4 py-2.5 text-sm font-bold focus:ring-2 ring-blue-500/30 outline-none transition-all" placeholder="书名" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase text-gray-500 flex items-center gap-2 px-1"><Globe2 size={12} className="text-blue-600" /> 作者</label>
                  <input required className="w-full bg-gray-100 dark:bg-white/5 border-none rounded-xl px-4 py-2.5 text-sm font-bold focus:ring-2 ring-blue-500/30 outline-none transition-all" placeholder="作者" value={formData.author} onChange={e => setFormData({ ...formData, author: e.target.value })} />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button type="button" onClick={() => setFormData({ ...formData, status: 'read' })} className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 transition-all ${formData.status === 'read' ? 'border-emerald-500 bg-emerald-500/5 text-emerald-600 shadow-sm' : 'border-transparent bg-gray-100 dark:bg-white/5 text-gray-400'}`}>
                    <CheckCircle2 size={16} /> <span className="font-bold text-xs">已读记录</span>
                  </button>
                  <button type="button" onClick={() => setFormData({ ...formData, status: 'unread' })} className={`flex-1 flex items-center justify-center gap-2 px-4 py-3 rounded-xl border-2 transition-all ${formData.status === 'unread' ? 'border-orange-500 bg-orange-500/5 text-orange-600 shadow-sm' : 'border-transparent bg-gray-100 dark:bg-white/5 text-gray-400'}`}>
                    <Circle size={16} /> <span className="font-bold text-xs">加入待读</span>
                  </button>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {BOOK_TYPES.map(type => (
                    <button key={type.id} type="button" onClick={() => setFormData({ ...formData, bookType: type.id })} className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all ${formData.bookType === type.id ? 'border-blue-600 bg-blue-600/5 text-blue-600 shadow-sm' : 'border-transparent bg-gray-100 dark:bg-white/5 text-gray-400 hover:bg-gray-200 dark:hover:bg-white/10'}`}>
                      <type.icon size={16} /> <span className="font-bold text-xs">{type.label}</span>
                    </button>
                  ))}
                </div>
              </div>
        ...
        <footer className="p-6 border-t border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-white/2 rounded-b-[32px] flex gap-3 shrink-0">
          ...
          <button type="submit" onClick={handleSubmit} disabled={loading} className="flex-[2] bg-blue-600 text-white px-8 py-3.5 rounded-xl font-black text-xs tracking-widest hover:bg-blue-700 shadow-xl shadow-blue-500/20 transition-all flex items-center justify-center gap-2">
