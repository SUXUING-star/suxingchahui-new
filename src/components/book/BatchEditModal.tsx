...
import { updateBooksBatch, compareBooksBatch } from "@/utils/bookApi";
import { BookResponse } from "@/models/BookReponse";

interface Props {
  onClose: () => void;
  onSuccess: () => void;
  existingBooks: BookResponse[]; // 传进当前页面显示的书，用来做对比
}
    ...
    <div className="fixed inset-0 z-[150] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-4xl bg-white dark:bg-[#0a0a0a] rounded-[40px] shadow-2xl border border-white/10 flex flex-col max-h-[90vh]">
        <header className="p-8 pb-4 flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-black italic text-blue-600">
              批量修改确认
            </h2>
            <p className="text-[10px] font-black text-gray-500 uppercase mt-1">
              {step === "input"
                ? "贴入你修改后的 JSON 数据"
                : `检测到 ${diffs.length} 条记录有改动`}
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <div className="space-y-4">
              {diffs.length === 0 ? (
                ...
                diffs.map((item, idx) => (
                  <div
                    key={idx}
                    className="bg-gray-50 dark:bg-white/5 p-4 rounded-2xl border border-white/5"
                  >
                    <div className="text-[11px] font-black text-blue-500 mb-2">
                      《{item.title}》
                    </div>
                    <div className="space-y-1">
                      {Object.entries(item.changes).map(([field, val]: any) => (
                        ...
                        >
                          <span className="text-gray-400 w-20 uppercase">
                            {field}
                          </span>
        ...
        <footer className="p-8 border-t border-white/5 flex gap-3">
          {step === "compare" && (
            <button
              ...
              className="flex-1 py-4 rounded-2xl font-bold text-gray-500"
            >
              返回重贴
            </button>
          )}
