...
import { Layers, Globe, Users, Clock, PieChart, X, BarChart3 } from 'lucide-react';
...
const BookStatsSidebar: React.FC<Props> = ({ stats, activeFilters, onFilterChange }) => {
  ...
  const renderItemsList = (items: StatItem[], regionName?: string, isModal = false) => {
    ...
    if (viewMode === 'type') {
      ...
      sortedItems.sort((a, b) => (b.read + b.unread) - (a.read + a.unread) || a.name.localeCompare(b.name));
    }
    ...
    return (
      <div className={`mt-3 ${isModal ? 'space-y-2.5' : 'space-y-3.5'} pl-3`}>
        {displayItems.map(item => {
          ...
          return (
            ...
            >
              <span className={`text-[11px] font-bold transition-colors ${isActive ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-700'}`}>
                {item.name}
              </span>
  ...
  // 渲染滚动主区域的内容
  const renderMainContent = (isModal: boolean) => (
    <div className={`flex-1 overflow-y-auto pr-1 mt-6 
      scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-white/10
      ${isModal ? 'pb-10' : ''}`}
    >
      <div className={isModal ? "space-y-8" : "space-y-8"}>
        {viewMode === 'type' && (
          <section className="border-b border-gray-100 dark:border-white/5 pb-6 mb-2">
            <div className="flex items-center gap-2 border-b border-gray-100 dark:border-white/5 pb-3">
              ...
              <h3 className="text-[10px] font-black text-gray-400 uppercase tracking-widest">阅读记录汇总</h3>
            </div>
        ...
        {sortedStats.map(region => (
          <section key={region.country}>
            <div 
              ...
              className={`flex justify-between items-end pb-3 cursor-pointer border-b transition-colors ${activeFilters.country === region.country ? 'border-blue-600' : 'border-gray-100 dark:border-white/5'}`}
            >
              <div className="flex items-center gap-2">
                <Globe size={14} className={activeFilters.country === region.country ? 'text-blue-600' : 'text-gray-400'} />
                <span className={`text-sm font-black ${activeFilters.country === region.country ? 'text-blue-600' : 'text-gray-600 dark:text-gray-200'}`}>
                  {region.country}
                </span>
              </div>
    ...
    <div className="flex bg-gray-100 dark:bg-white/5 p-1 rounded-xl shrink-0">
      {[
        ...
        <button 
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-[10px] font-black transition-all ${viewMode === m.id ? 'bg-white dark:bg-white/10 shadow-sm text-blue-600' : 'text-gray-400 hover:text-gray-500'}`}
        >
  ...
  return (
    <>
          ...
          <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-lg h-[80vh] bg-[#f8f9fa] dark:bg-[#0f0f0f] rounded-[32px] shadow-2xl flex flex-col overflow-hidden p-6">
              <header className="flex items-center justify-between px-2 pb-4 shrink-0">
                ...
                <button onClick={() => setIsModalOpen(false)} className="p-2 bg-gray-100 dark:bg-white/5 rounded-full hover:bg-gray-200 transition-colors">
                  <X size={18} />
                </button>
              </header>
              {renderTabs()}
      ...
      {/* 2. 桌面端展示：核心改进点在这里 */}
      <aside className="hidden md:block w-full sticky top-6 h-[calc(100vh-60px)]">
