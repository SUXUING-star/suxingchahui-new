...
const UserMenu: React.FC<UserMenuProps> = ({ setIsProfileModalOpen }) => {
            ...
            {isOpen && (
                <div className="absolute right-0 mt-3 w-64 bg-white/95 dark:bg-gray-800/95 backdrop-blur-2xl rounded-3xl shadow-[0_20px_50px_rgba(0,0,0,0.3)] py-3 z-[100] border border-gray-100 dark:border-white/5 animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="px-2 space-y-1">
                        <Link to="/my-posts" onClick={() => setIsOpen(false)} className="flex items-center px-4 py-3 text-sm font-black text-gray-600 dark:text-gray-300 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 rounded-2xl transition-colors">
                            <LayoutDashboard size={18} className="mr-3 text-blue-500" /> 我的投稿
                        </Link>

                        {user.isAdmin && (
                            <Link to="/admin/pending" onClick={() => setIsOpen(false)} className="flex items-center px-4 py-3 text-sm font-black text-gray-600 dark:text-gray-300 hover:bg-red-50 dark:hover:bg-red-900/20 hover:text-red-600 rounded-2xl transition-colors">
                                <ShieldCheck size={18} className="mr-3 text-red-500" /> 审核后台
                            </Link>
                        )}

                        <button onClick={() => { setIsProfileModalOpen(true); setIsOpen(false); }} className="flex items-center w-full px-4 py-3 text-sm font-black text-gray-600 dark:text-gray-300 hover:bg-emerald-50 dark:hover:bg-emerald-900/20 hover:text-emerald-600 rounded-2xl transition-colors text-left">
                            <ImagePlus size={18} className="mr-3 text-emerald-500" /> 更换头像
                        </button>
                    </div>

                    <div className="mt-2 px-2 border-t dark:border-white/5 pt-2">
                        <button onClick={handleLogout} className="flex items-center w-full px-4 py-3 text-sm font-black text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-2xl transition-colors text-left">
