...
const LeftSidebar: React.FC = () => {
  ...
  return (
    ...
    >
      <div
        className={`bg-white/85 dark:bg-gray-900/90 backdrop-blur-2xl shadow-xl border border-white/40 dark:border-white/5 transition-all duration-500 overflow-hidden flex flex-col ${
        ...
        >
          <div className="space-y-4">
            {recentPosts.map((post) => (
              <article
                key={post.id}
                className="group flex gap-3 transition-all active:scale-95"
              >
                {post.coverImage?.src && (
                  <Link to={`/post/${post.id}`} className="flex-shrink-0">
                    <LazyImage
                      ...
                      alt={post.title}
                      wrapperClassName="w-12 h-12 rounded-xl shadow-sm group-hover:shadow-md transition-all overflow-hidden"
                    />
                  </Link>
                )}
