...
export const apiSendVerificationCode = (payload: {
  email: string;
  type: "register" | "reset_password";
}): Promise<BaseResponse> => apiPost("/auth/send-verification", payload);

export const apiCheckEmailExists = (
  email: string,
): Promise<CheckEmailResponse> => apiPost("/auth/forgot-password", { email });

export const apiResetPassword = (payload: any): Promise<BaseResponse> =>
  apiPut("/auth/reset-password", payload, null);

export const apiUpdateNickname = (
  nickname: string,
  token: string,
