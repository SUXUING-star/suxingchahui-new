...
type TagMap = Record<string, PostResponse[]>;
...
const Tags: React.FC = () => {
  ...
  useEffect(() => {
    if (!loading && Object.keys(tags).length > 0) {
      // 2. 更改为 v4 的 animate 写法，并使用直接导入的 stagger 和 ease 参数名
      animate(".tag-btn-animate", {
        scale: [0.8, 1],
        opacity: [0, 1],
        delay: stagger(30),
        duration: 600,
  ...
  const handleTagClick = (tag: string) => {
    ...
    navigate(newTag ? `#${encodeURIComponent(newTag)}` : location.pathname, {
      replace: true,
    });
  };
  ...
  if (error)
    return (
      <StatusPlaceholder type="error" title="标签云坍塌" onRetry={fetchTags} />
    );
  if (Object.keys(tags).length === 0)
    return (
      <StatusPlaceholder type="empty" title="暂无标签" onRetry={fetchTags} />
    );
      ...
      <div className="flex flex-wrap items-center justify-center gap-4 mb-20">
        {Object.entries(tags).map(([tag, posts]) => (
          ...
          >
            ...
            {tag}{" "}
            <span className="ml-2 opacity-40 text-[10px]">{posts.length}</span>
          </button>
        ))}
      </div>
