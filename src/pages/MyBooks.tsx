...
const MyBooks: React.FC = () => {
  ...
  const [detailBook, setDetailBook] = useState<BookResponse | null>(null);
    ...
    <div className="w-full max-w-7xl mx-auto px-4 py-6 text-gray-900 dark:text-white min-h-screen pb-32">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 mb-10">
        <div className="flex items-center gap-3 px-4 py-3 rounded-[24px] bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-sm shrink-0">
          <button
            onClick={() => navigate("/")}
            className="p-2 hover:bg-gray-100 dark:hover:bg-white/10 rounded-full transition-colors flex items-center gap-1 group"
          >
            <ArrowLeft
              size={20}
        ...
        <div className="flex flex-wrap items-center gap-2 md:gap-3 px-4 py-3 rounded-[24px] bg-white/60 dark:bg-white/5 backdrop-blur-xl border border-white/40 dark:border-white/10 shadow-sm">
          ...
          {isSelectMode ? (
            <div className="flex items-center gap-2">
              <button
                onClick={execDelete}
                className="bg-red-500 text-white px-4 py-2 rounded-lg font-bold text-[11px]"
              >
                删除
              </button>
              <button
                ...
                className="px-3 py-2 text-[11px] font-bold text-gray-500"
              >
                取消
              </button>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="flex items-center gap-2 bg-white/50 dark:bg-white/5 px-4 py-2 rounded-xl font-bold text-[11px] border border-gray-100 dark:border-transparent transition-all hover:bg-gray-100"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 bg-blue-600 text-white px-5 py-2 rounded-xl font-bold text-[11px] hover:bg-blue-700 shadow-md transition-all active:scale-95"
              >
                <Plus size={16} /> 记录
              </button>
            </div>
            ...
            <>
              ...
              >
                {fetchingNextPage ? (
                  ...
                  !hasMore &&
                  books.length > 0 && (
                    <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.3em] opacity-30">
                      已经到底了
                    </p>
                  )
                )}
