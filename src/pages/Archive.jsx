...
function Archive() {
  ...
  useEffect(() => {
    if (!loading && Object.keys(archiveData).length > 0) {
        anime({ targets: '.archive-animate', translateY: [30, 0], opacity: [0, 1], delay: anime.stagger(100), duration: 800, easing: 'easeOutExpo' });
    }
  }, [loading, archiveData]);
            ...
            <div className="space-y-16 pl-4 sm:pl-10 border-l-2 border-gray-200 dark:border-white/10">
              {Object.entries(months).sort((a, b) => b[0] - a[0]).map(([month, posts]) => (
                <div key={month} className="space-y-8">
                  ...
                  {/* 核心修正：Grid 布局限制。
                      在带侧边栏的情况下，PC端建议 2列 或 3列。
                      grid-cols-4 会把 300px 的卡片挤死。
                  */}
