...
export const NotificationProvider: React.FC<{ children: ReactNode }> = ({
  ...
  const closeNotification = useCallback((id: number) => {
    // 2. 升级为 v4 写法：easing -> ease, complete -> onComplete
    animate(`.notification-item[data-id="${id}"]`, {
      translateX: 400,
      opacity: 0,
      duration: 300,
      ease: "outQuad",
  ...
  const showNotification = useCallback(
    (message: string, type: NotificationType = "success", title = "") => {
      const id = Date.now();
  ...
  useEffect(() => {
    if (notifications.length > 0) {
      ...
      // 3. 升级为 v4 的 animate 方法
      animate(`.notification-item[data-id="${lastId}"]`, {
        opacity: [0, 1],
        translateX: [100, 0],
        duration: 400,
        ease: "outBack",
